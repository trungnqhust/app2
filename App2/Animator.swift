...
class Animator: NSObject, UIViewControllerAnimatedTransitioning {
    ...
    func animateTransition(using context: UIViewControllerContextTransitioning) {
        // push
        if let discoverVC = context.viewController(forKey: UITransitionContextViewControllerKey.from) as? MusicListViewController,
            let detailVC = context.viewController(forKey: UITransitionContextViewControllerKey.to) as? MusicListViewController {
            moveFromCategories(discoverVC: discoverVC, toList: detailVC, withContext: context)
        }
            
            // pop
        else if let discoverVC = context.viewController(forKey: UITransitionContextViewControllerKey.to) as? MusicListViewController,
            let detailVC = context.viewController(forKey: UITransitionContextViewControllerKey.from) as? MusicListViewController {
            moveFromList(detailVC: detailVC, toDiscover: discoverVC, withContext: context)
        }
    }
    
    private func moveFromCategories(discoverVC: MusicListViewController, toList detailVC: MusicListViewController, withContext context: UIViewControllerContextTransitioning) {
    ...
    private func moveFromList(detailVC: MusicListViewController, toDiscover discoverVC: MusicListViewController, withContext context: UIViewControllerContextTransitioning) {
